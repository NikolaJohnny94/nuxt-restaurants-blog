    ...
    <div>
        ...
        <div v-if="restaurant.name || restaurant.description">
            ...
            <div class="ml-3" v-for="review in reviews" :key="review._id">
                <NuxtLink
                    :to="{ name: 'person-slug', params: { slug: review.name.split(' - ')[1], id: review.persons[0] } }"
                >
...
export default {
    ...
    async fetch() {
        let restaurantID
        const restaurantsResponse = await this.$axios.$get(
            `https://strapi-restaurants.herokuapp.com/restaurants`
        );
        restaurantsResponse.forEach((restaurant) => {
            if (restaurant.name.replace(" ", "-").toLowerCase() === this.$route.params.slug) {
                restaurantID = restaurant.id;
            }
        });
        const res = await this.$axios
            .$get(`${process.env.API}/restaurants/${restaurantID}`)
            .then((res) => {
                this.restaurant = res
